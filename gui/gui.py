...
def set_all_tools(board, start_pos, is_left_board):
    ...
    if is_left_board:
        ...
        for name in names:
            images.append(pygame.image.load('images/b' + name + '.png'))
        for name in names:
            images.append(pygame.image.load('images/w' + name + '.png'))
    ...
    pos = start_pos[:]
    for img in images[:5]:
        board[0][cnt] = tools[cnt](img, pos[:], black)
        pos[0] += square_size
        cnt += 1
    ...
    pos = [75, 150]
    for i in range(8):
        board[1][i] = Pawn(images[5], pos[:], black)
        pos[0] += square_size
    cnt = 0
    pos = [75, 450]
    for img in images[6:-1]:
        board[-1][cnt] = tools[cnt](img, pos[:], white)
        pos[0] += square_size
        cnt += 1
    ...
    pos = [75, 400]
    for i in range(8):
        board[-2][i] = Pawn(images[11], pos[:], white)
        pos[0] += square_size
...
def game(board, transplant_pieces, start_pos, transplant_start_pos):
    stop = False
    moving = False
...
t1.start()
t2.start()'''
game(board, transplant_pieces, start_pos[:], transplant_start_pos[:])
game(board2, transplant_pieces2, start_pos2[:], transplant_start_pos2[:])
pygame.quit()
